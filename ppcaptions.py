...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="ppcaptions", description="Uses AI to locally transcribes speech from media files, generating subtitle files, translates the generated subtitles, inserts them into the mp4 container, and burns them directly into video",
                                     argument_default=True, allow_abbrev=True, add_help=True, usage='ppcaptions -i INPUT_PATH [other options]')
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.output_softsubs:
        if args.input_path.is_file():
            ...
            args.output_softsubs = compatibility_path if (compatibility_path := Path(args.input_path.parent, "ppcaptions_srt_" + args.input_path.name)).exists() else Path(args.input_path.parent, "softsubs_" + args.input_path.name)
    if not args.output_hardsubs:
        if args.input_path.is_file():
            args.output_hardsubs = Path(args.input_path.parent, "hardsubs")
        else:
            args.output_hardsubs = compatibility_path if (compatibility_path := Path(args.input_path.parent, "ppcaptions_burned_" + args.input_path.name)).exists() else Path(args.input_path.parent, "hardsubs_" + args.input_path.name)
        ...
        for path in files_iterator:
            ...
            with time_task(message_start=f"\nProcessing {yellow}{rel_path.as_posix()}{default}", end="\n", message="⌚ Done in"):
                try:
                    ...
                    print(f"{yellow}check ppcaptions-errors.txt for details{default}")
                    current_time = time.strftime("%y/%m/%d %H:%M:%S", time.localtime())

                    error_message = f"[{current_time}] {file}: {type(e).__name__}: {str(e)}"
                    with open(Path(Path(getframeinfo(currentframe()).filename).resolve().parent, "ppcaptions-errors.txt"), "a") as f:
