...
function App() {
  ...
  const AUTH_PASSWORD_CACHE_KEY = 'ppcaptions_auth_password'

  const i18n = useMemo(
    () => ({
      pt: {
        appEyebrow: 'ppcaptions Web',
        appTitle: 'Traduza, resuma e edite ppcaptionsdas em segundos.',
        appLead:
        ...
        editedApplied: 'ppcaptionsda editada aplicada.',
        preparingBatches: 'Preparando lotes...',
        processingBatch: 'Processando lote',
        assemblingSubtitle: 'Montando ppcaptionsda',
        loadingCaptions: 'ppcaptionsdas carregadas',
        ...
        downloadSubtitle: 'Baixar ppcaptionsda',
        apply: 'Aplicar',
  ...
  useEffect(() => {
    const savedTranslate = readCache<TranslateConfig>('ppcaptions_translate_config')
    const savedTltw = readCache<TltwConfig>('ppcaptions_tltw_config')
    const savedTool = readCache<Tool>('ppcaptions_active_tool')
    const savedUiLanguage = readCache<UiLanguage>('ppcaptions_ui_language')
  ...
  useEffect(() => {
    writeCache('ppcaptions_translate_config', translateConfig)
  }, [translateConfig])

  useEffect(() => {
    writeCache('ppcaptions_tltw_config', tltwConfig)
  }, [tltwConfig])

  useEffect(() => {
    writeCache('ppcaptions_active_tool', activeTool)
  }, [activeTool])

  useEffect(() => {
    writeCache('ppcaptions_ui_language', uiLanguage)
  }, [uiLanguage])

  useEffect(() => {
    if (isAuthenticated) {
      writeCache('ppcaptions_is_authenticated', true)
    }
  }, [isAuthenticated])
          ...
          <div className="auth-card panel">
            ...
            <div className="auth-support-box">
              ...
              <a
                ...
                href="https://livepix.gg/ppcaptionsdonate/grupo-dos-apoiadores"
                target="_blank"
                rel="noreferrer"
              >
