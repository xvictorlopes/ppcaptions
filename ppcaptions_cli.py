"""Console entry point for the PU Prime Captions CLI."""
...
_LEGACY_WARNING = (
    "Warning: running legacy PU Prime Captions CLI fallback because the installed package "
...
def main(argv: Sequence[str] | None = None) -> None:
    """Run the packaged PU Prime Captions CLI and propagate its exit code."""
    ...
    if forwarded_args is not None:
        ...
        program_name = original_sys_argv[0] if original_sys_argv else "ppcaptions"
        sys.argv = [program_name, *map(str, forwarded_args)]

    try:
        module = importlib.import_module("ppcaptions")
    except Exception:
